...
class GroupUpload extends Component {
  ...
  handlePictureSeletorOK() {  // 确定当前预览图片可以上传
    if (this.store.selectImgNum + this.state.selectedPictureList.length < 11) {
      runInAction(() => {
        this.store.selectImgNum += this.state.selectedPictureList.length;
        this.store.setUrlList()   // 清空图片 的url地址
      })
      setTimeout(() => {
      ...
      runInAction(() => {
        this.store.pic_num_corning = true;
      })
      setTimeout(() => {
        runInAction(() => {
          this.store.pic_num_corning = false;
        })
      }, 3000)
      return false;
    }
  ...
  getPictures(listPicture) { //上传完毕
    // console.log('上传完毕', listPicture );
    let newPictures = listPicture.map(item => {
      if (typeof (item.url) != undefined) {
        /*if(item.response && item.response.status == 2000){
            let a = item.url + '&_width=' + item.response.data.width + '&_height' + item.response.data.height;
            return a;
  ...
  render() {
    ...
    return (
      <span>
        ...
        {
          this.state.showPictureSeletor ? <Modal
            ...
            <div className="picture-list">
              {this.state.pictureList.length === 0
                ? <div style={{ height: '28px', lineHeight: '28px' }}>{lang.pleaseUploading}</div>
