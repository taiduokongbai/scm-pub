...
class UploadImage extends Component {
  ...
  onChange(info) {
    ...
    if (this.props.fileType == "image") {
      ...
      // url = 'window.LUX_FILEUP/announcement/1JQHYx2JEjC508127335530.jpg'
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.uploadProps) {
      // console.log('nextProps.uploadProps.fileList', nextProps.uploadProps.fileList);
      this.state.upReceiverFun = setTimeout(() => {
        //限制频繁调用组件制定的cbReceiver
  ...
  render() {
    ...
    return (
      <div>
        ...
        {
          this.store.pic_num_corning ?
            <span style={{ display: 'block', color: 'red', marginLeft: '10px', height: '28px', lineHeight: '28px' }}>最多上传10张图片</span> :
        ...
        {/*<span style={{ display: this.store.pic_num_corning ? 'block' : 'none', color: 'red', marginLeft: '10px', height: '28px', lineHeight: '28px' }}>最多上传10张图片</span>
        <span>{this.props.lang.limitAndTypeTip.replace("$limit$", this.props.limit).replace("$type$", PRO_QINIU.supportMime[this.props.fileType].join(", "))}</span>*/}
      </div>
    )
  }
