...
class ImgStyleControls extends Component {
  ...
  getImgObject(fileObj) {
    // console.log(fileObj, 'getImgObject in mediaimaguploader');
  ...
  successLoading(type) {
    ...
    let pfopImages = this.state.images.map((item) => {
      // console.log(item, 'successLoading in mediaimaguploader');
      ...
      <div className="RichEditor-controls">
        ...
        <GroupUpload
          ...
          imageList={this.state.images.map((item) => {
            //console.log(item, 'groupUpload imageList-------------------');
            item.url
          })}
          atuoSize={[650, 0]}
        ...
        <span className={className} onClick={that.onImgToggle} style={{display:'none'}}>
        ...
        <div
          style={{
            ...
            overflow: "hidden",
            position: "absolute"
          }}>{this.state.images.map((item, index) => <img style={{ width: "100px" }} key={index} src={
            item.url + "?t=10"
          }
            onError={
