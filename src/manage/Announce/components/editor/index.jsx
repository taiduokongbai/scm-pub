...
class EditorConcist extends React.Component {
  ...
  render() {
    let urlInput;
    // ref="urltext"
    if (this.state.showURLInput) {   // 添加链接modal
      urlInput = <Modal
    ...
    return (
      <div className="RichEditor-root editorHidden" content={this.state.HTML} id="text-editor-container">
        <div>
          ...
          {this.state.showMarkdownSource == false && this.props.image && <ImgStyleControls store={this.store} uploadConfig={this.props.uploadConfig} receiveImage={this.addImage} watermarkImage={this.props.watermarkImage} lang={lang[this.state.language]}
...
const Image = (props) => {
  /*let url = '';
  if(props.url.include('&_width') || props.url.include('&_height')){
    url = props.url.split('&_')[0]
  }else url = props.url;*/
  return <img src={props.url} className="media" style={{maxWidth: 950}}/>;
};
