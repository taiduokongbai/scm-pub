...
class SiteCont extends Component {
    constructor(props, context) {
        super(props, context);
         this.state = {
            searchPm: {
                status: -1, siteName: '', siteCode: '', page: 1, pageSize: 15
            }
        };
    }
    ...
    tablePaging = (page) => {
        ...
        if (!tabLoading){
            ...
            let newSearchPm = JSON.parse(JSON.stringify(this.state.searchPm));
            if (newSearchPm.status == '-1') {
                delete newSearchPm.status
            }
            SiteList(newSearchPm).then(json => {
                if (json.status === 2000) {
...
const mapDispatchToProps = (dispatch) => ({
    ...
    AddressCodeRule: () => dispatch(SiteAct.AddressCodeRule({businessIndex:13})),
})
