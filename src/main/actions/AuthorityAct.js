...
import  SelectorAct from  './SelectorAct';
let AuthorityAct = {
    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@调接口@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
        ...
        ReqApi.get({
            ...
            if(res.data !==null ) {
                let state = getState()[AUTHORITYREDU]
                    ...
                    .setIn(["authPaging", "current"], res.data.page)
    ...
    getAuthorityDetail:(id)=>(dispatch,getState)=>{
        ...
        ReqApi.get({
            ...
            state = getState()[AUTHORITYREDU]
                .setIn(["authDetail"], res.data)
                .setIn(["treeChecked"], res.data.resources)
                .setIn(["poses"], res.data.poses)
                .setIn(["resourceParents"], res.data.resourceParents);
            dispatch({type:AUTHORITYREDU , state});
        });
    },
    cleanResourceParents:()=>(dispatch,getState)=>{
        let state = getState()[AUTHORITYREDU].setIn(["resourceParents"],[]);
        dispatch({type:AUTHORITYREDU ,state});
    },
    ...
    getMenberList:(pm)=>(dispatch,getState)=>{
        ...
        ReqApi.get({
            ...
            if(res.data !==null) {
                let state = getState()[AUTHORITYREDU]
                    .setIn(["menData"],[].concat(res.data.list))
    ...
    addMenbers:(pm = {})=>(dispatch,getState)=>{
        ...
        return ReqApi.post({
            ...
            if(res.status === 2000){
                dispatch(AuthorityAct.getMenberList(getState()[AUTHORITYREDU].get("menSearch").toJS()));
                dispatch(SelectorAct.visibleDialog(false));
                dispatch(AuthorityAct.setSideVisible(true));
                dispatch(AuthorityAct.getAuthorityList(getState()[AUTHORITYREDU].get("authSearch").toJS()));
                message.success("新建权限人成功");
            }
            return res;
        });
    },
    ...
    //@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@设置值@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    setTreeChecked: (keys, poses) => (dispatch, getState) => {
        let state = getState()[AUTHORITYREDU].setIn(["treeChecked"],keys).setIn(["poses"],poses);
        dispatch({type:AUTHORITYREDU ,state});
    },
