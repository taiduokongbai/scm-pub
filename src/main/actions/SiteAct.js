...
const actions = {
        ...
        return ReqApi.get({
            ...
            if (json.status === 2000) {
                ...
                if (json.data.list.length) {
                    dispatch(actions.OrgList({ orgCode: _data.orgCode, page: 1, pageSize: 10 }));
   ...
   OrgList: (pm = {}) => (dispatch, getState) => {
    ...
    AddressCodeRule: (pm = {}) => (dispatch, getState) => {
        return ReqApi.get({
            url: Urls.GET_CODERULE,
            pm,
            type: 'AddressCodeRule'
        }).then(json => {
            if (json.status === 2000) {
                let state = getState()[SITEREDU].set('addressCodeRule', json.data.ruleType)
                dispatch({ type: SITEREDU, state });
            }
            return json;
        })
