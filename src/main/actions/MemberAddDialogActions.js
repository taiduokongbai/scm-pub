...
//职位列表
export let getPositionListEnum =  (val="") => (dispatch,getState)=>{
    return ReqApi.get({
...
//获取是否自动获取编码规则
export let getCodeRuleEnum =  () => (dispatch,getState)=>{
    return ReqApi.get({
        url: Urls.PUB_CODE_RULE_IS_AUTO,
        pm:{
            businessIndex:6
        }
    }).then(json => {
        if(json.status=== 2000 &&  json.data){
            let state =  getState()[MEMBERDIALOGREDU].setIn(["addMemberDialog","codeRuleIsAuto"],json.data.ruleType);
            dispatch({type:MEMBERDIALOGREDU, state});
        }
    })
}
...
//编码规则
export let getCodeRule =  (pm) => (dispatch,getState)=>{
    return ReqApi.get({
