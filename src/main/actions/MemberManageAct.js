...
const actions = {
    ...
    GetInfoList: (val) => (dispatch, getState) => {
        let { list, page, pageSize, total, deptName, deptMgr, deptNum, sonDeptNum } = val;
        let pm = {
            deptName: deptName,
            deptMgr: deptMgr,  
            deptNum: deptNum, // 部门人数
            sonDeptNum: sonDeptNum    // 子部门个数
        }
        let state = getState()[MEMBERCHANGEREDU].set('dataSource', list).set('paging', { page, pageSize, total }).set('headerBar_visible', ["none", "block"]).set('headerInfo', pm);
        dispatch({ type: MEMBERCHANGEREDU, state })
    },
    ...
    stopAccount: (pm = {}) => (dispatch, getState) => {
        ...
        ReqApi.post({
            ...
            if (json.status == 2000) {
                // dispatch(actions.getMemberInfoList({ deptCode: deptCode, employeeName: '', phone: '', page: 1, pageSize: 15 }));
