...
const actions = {
    ...
    detail:(pm={},type)=>(dispatch, getState)=>{
     ...
     get_detail:(data)=>(dispatch, getState)=>{
        let state = getState()[AT.DEPARTMENTREDU].set('detailAddressName', "");
        dispatch({ type: AT.DEPARTMENTREDU, state })
        if(data.addrCode){
             dispatch(actions.get_addDetail({"addressCode":data.addrCode}));
        }
        state=getState()[AT.DEPARTMENTREDU].set('detail',data);
        dispatch({type: AT.DEPARTMENTREDU, state})
    },
    ...
    get_addDetail:(pm={})=>(dispatch,getState)=>{
        return ReqApi.get({
            ...
            if(json.status==2000&&json.data.list.length>0){
                let address=json.data.list[0].countryName+""+json.data.list[0].cityName+""+json.data.list[0].countyName+""+json.data.list[0].addressDetl
                let state = getState()[AT.DEPARTMENTREDU].set('detailAddressName', address);
                dispatch({ type: AT.DEPARTMENTREDU, state })
            }
        })
    },
    GetCodeRule: (pm = {}) => (dispatch, getState) => {
        return ReqApi.get({
            ...
            if (json.status === 2000) {
                let state = getState()[AT.DEPARTMENTREDU].set('codeRule', json.data.ruleType)
                dispatch({ type: AT.DEPARTMENTREDU, state });
            }
            return json;
        })
