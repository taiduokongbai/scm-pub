...
import AddCurrencyComp from '../dialogComp/AddCurrencyComp';
import EditCurrencyComp from '../dialogComp/EditCurrencyComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class CurrencyComp extends Component {
    constructor(props, context) {
        ...
        this.columns = [
            ...
            },
            {
                ...
                key:"standardCoin",
                render: (txt)=>window.ENUM.getEnum("standardCoin", txt+'')
                ...
                fixed:'right',
                className:'operate',
                render: (text, record, index) => {if(record.type==2){ return this.renderTableButton(text, record, index)}
                else {
    ...
    renderNotOperate=()=>{
        return (
            <span>
                <span className="three-line"><span className="one-line">--</span><span className="one-line">--</span><span className="one-line">--</span></span>
            </span>
        )
    }
    ...
    handleDelete = (e, record, index) => {
        currencyStore.deleteCurrencyItem({id: record.id})
    }
    handleEdit = (e, record, index) => {
        currencyStore.getDetail({id: record.id})
    }
    handlerIsDisable = (id, status) => {
        currencyStore.isDisableCurrencyItem({id: id, status: status})
    }
    onChange = (value) => {
        currencyStore.setSearchKey(value)
        currencyStore.setInput(value)
    };
    handleStatusChange = (value) => {
        currencyStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        currencyStore.setSearchVal(event.target.value)
    };
    query = () => {
        currencyStore.fetchTableList();
    }
    componentDidMount() {
        currencyStore.fetchTableList();
    }
    componentWillUnmount(){
        currencyStore.resetMobx();
    }
        ...
        return (
            <div className="baseData-content">
                <div className='search-content'>
                    <Select style={{width: 180, height: 30}} onChange={this.onChange} defaultValue="curCode">
                        <Option value="curCode">编码</Option>
                        <Option value="curName">名称</Option>
                        <Option value="status">状态</Option>
                    </Select>
                    {currencyStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={currencyStore.searchVal} onChange={this.handleInputChange}/> :
                ...
                />
                { currencyStore.visible? <AddCurrencyComp/>:null}
                { currencyStore.editVisible? <EditCurrencyComp/>:null}
            </div>
        )
    }
}
