...
import { renzhiStore } from '../stores/RenzhiStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class RenzhiComp extends Component {
    ...
    onChange = (value) => {
        renzhiStore.setSearchKey(value)
        renzhiStore.setInput(value)
    };
    handleStatusChange = (value) => {
        renzhiStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        renzhiStore.setSearchVal(event.target.value)
    };
    query = () => {
        renzhiStore.fetchTableList();
    }
                ...
                <div className='search-content'>
                    ...
                    {renzhiStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={renzhiStore.searchVal} onChange={this.handleInputChange}/> :
