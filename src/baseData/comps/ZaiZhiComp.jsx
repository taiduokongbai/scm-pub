...
import { zaizhiStore } from '../stores/ZaizhiStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class ZaiZhiComp extends Component {
    ...
    onChange = (value) => {
        zaizhiStore.setSearchKey(value)
        zaizhiStore.setInput(value)
    };
    handleStatusChange = (value) => {
        zaizhiStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        zaizhiStore.setSearchVal(event.target.value)
    };
    query = () => {
        zaizhiStore.fetchTableList();
    }
                ...
                <div className='search-content'>
                    ...
                    {zaizhiStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={zaizhiStore.searchVal} onChange={this.handleInputChange}/> :
