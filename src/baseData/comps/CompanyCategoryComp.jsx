...
import { companycategoryStore } from '../stores/CompanycategoryStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class CompanyCategoryComp extends Component {
    ...
    onChange = (value) => {
        companycategoryStore.setSearchKey(value)
        companycategoryStore.setInput(value)
    };
    handleStatusChange = (value) => {
        companycategoryStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        companycategoryStore.setSearchVal(event.target.value)
    };
    query = () => {
        companycategoryStore.fetchTableList();
    }
    ...
    componentWillUnmount(){
        companycategoryStore.resetMobx();
    }
    render() {
        return (
            <div>
                <div className='search-content'>
                    ...
                    {companycategoryStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={companycategoryStore.searchVal} onChange={this.handleInputChange}/> :
...
export default CompanyCategoryComp;