...
import { xuexingStore } from '../stores/XuexingStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class XueXingComp extends Component {
    ...
    onChange = (value) => {
        xuexingStore.setSearchKey(value)
        xuexingStore.setInput(value)
    };
    handleStatusChange = (value) => {
        xuexingStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        xuexingStore.setSearchVal(event.target.value)
    };
    query = () => {
        xuexingStore.fetchTableList();
    }
        ...
        return (
            <div>
                <div className='search-content'>
                    ...
                    {xuexingStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={xuexingStore.searchVal} onChange={this.handleInputChange}/> :
