...
import AddShoufukuanComp from '../dialogComp/AddShoufukuanComp';
import EditShoufukuanComp from '../dialogComp/EditShoufukuanComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class ShoufukuanComp extends Component {
    ...
    handleDelete = (e, record, index) => {
        shoufukuanStore.deleteCategoryItem({id: record.id})
    }
    handleEdit = (e, record, index) => {
        shoufukuanStore.getDetail({id: record.id})
    }
    handlerIsDisable = (id, status) => {
        shoufukuanStore.isDisableCategoryItem({id: id, status: status})
    }
    onChange = (value) => {
        shoufukuanStore.setSearchKey(value)
        shoufukuanStore.setInput(value)
    };
    handleStatusChange = (value) => {
        shoufukuanStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        shoufukuanStore.setSearchVal(event.target.value)
    };
    query = () => {
        shoufukuanStore.fetchTableList();
    }
    add = () => {
        shoufukuanStore.showModal();
    }
    componentDidMount() {
        shoufukuanStore.fetchTableList();
    }
    componentWillUnmount(){
        shoufukuanStore.resetMobx();
    }
    render() {
        return (
            <div className="baseData-content">
                <div className='search-content'>
                    ...
                    {shoufukuanStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={shoufukuanStore.searchVal} onChange={this.handleInputChange}/> :
                ...
                <Table
                    {...shoufukuanStore.Props}
                    rowKey='catCode'
                    columns={this.columns}
                />
                { shoufukuanStore.visible? <AddShoufukuanComp/>:null}
                { shoufukuanStore.editVisible? <EditShoufukuanComp/>:null}
            </div>
        )
    }
}
export default ShoufukuanComp;