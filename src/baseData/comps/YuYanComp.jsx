...
import { yuyanStore } from '../stores/YuyanStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class YuYanComp extends Component {
    ...
    onChange = (value) => {
        yuyanStore.setSearchKey(value)
        yuyanStore.setInput(value)
    };
    handleStatusChange = (value) => {
        yuyanStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        yuyanStore.setSearchVal(event.target.value)
    };
    query = () => {
        yuyanStore.fetchTableList();
    }
                ...
                <div className='search-content'>
                    ...
                    {yuyanStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={yuyanStore.searchVal} onChange={this.handleInputChange}/> :
