...
import { xingbieStore } from '../stores/XingbieStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class XingBieComp extends Component {
    ...
    onChange = (value) => {
        xingbieStore.setSearchKey(value)
        xingbieStore.setInput(value)
    };
    handleStatusChange = (value) => {
        xingbieStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        xingbieStore.setSearchVal(event.target.value)
    };
    query = () => {
        xingbieStore.fetchTableList(Object.assign({},{[xingbieStore.searchKey]:xingbieStore.searchVal},{subCode: "C001"}));
    }

    componentDidMount() {
        xingbieStore.fetchTableList();
    }
    componentWillUnmount(){
        xingbieStore.resetMobx();
    }
    render() {
        return (
            <div>
                <div className='search-content'>
                    ...
                    {xingbieStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={xingbieStore.searchVal} onChange={this.handleInputChange}/> :
