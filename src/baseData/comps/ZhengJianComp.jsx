...
import { zhengjianStore } from '../stores/ZhengjianStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class ZhengJianComp extends Component {
    ...
    onChange = (value) => {
        zhengjianStore.setSearchKey(value)
        zhengjianStore.setInput(value)
    };
    handleStatusChange = (value) => {
        zhengjianStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        zhengjianStore.setSearchVal(event.target.value)
    };
    query = () => {
        zhengjianStore.fetchTableList();
    }
        ...
        return (
            <div>
                <div className='search-content'>
                    ...
                    {zhengjianStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={zhengjianStore.searchVal} onChange={this.handleInputChange}/> :
