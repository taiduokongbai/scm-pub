...
import { xueliStore } from '../stores/XueliStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class XueLiComp extends Component {
    ...
    onChange = (value) => {
        xueliStore.setSearchKey(value)
        xueliStore.setInput(value)
    };
    handleStatusChange = (value) => {
        xueliStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        xueliStore.setSearchVal(event.target.value)
    };
    query = () => {
        xueliStore.fetchTableList();
    }
                ...
                <div className='search-content'>
                    ...
                    {xueliStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={xueliStore.searchVal} onChange={this.handleInputChange}/> :
