...
import { companynatureStore } from '../stores/CompanynatureStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class CompanyNatureComp extends Component {
    ...
    onChange = (value) => {
        companynatureStore.setSearchKey(value)
        companynatureStore.setInput(value)
    };
    handleStatusChange = (value) => {
        companynatureStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        companynatureStore.setSearchVal(event.target.value)
    };
    query = () => {
        companynatureStore.fetchTableList();
    }
        ...
        return (
            <div>
                <div className='search-content'>
                    ...
                    {companynatureStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={companynatureStore.searchVal} onChange={this.handleInputChange}/> :
...
export default CompanyNatureComp;