...
import { hangyeStore } from '../stores/HangyeStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class HangYeComp extends Component {
    ...
    onChange = (value) => {
        hangyeStore.setSearchKey(value)
        hangyeStore.setInput(value)
    };
    handleStatusChange = (value) => {
        hangyeStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        hangyeStore.setSearchVal(event.target.value)
    };
    query = () => {
        hangyeStore.fetchTableList();
    }
                ...
                <div className='search-content'>
                    ...
                    {hangyeStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={hangyeStore.searchVal} onChange={this.handleInputChange}/> :
