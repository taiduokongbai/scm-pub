...
import { hunyinStore } from '../stores/HunyinStore';
import { Select,Input,Button } from '../../base/components/AntdComp';
const Option = Select.Option;
let { observer } = mobxReact;
@observer
class HunYinComp extends Component {
    ...
    onChange = (value) => {
        hunyinStore.setSearchKey(value)
        hunyinStore.setInput(value)
    };
    handleStatusChange = (value) => {
        hunyinStore.setSearchVal(value)
    };
    handleInputChange = (event) => {
        hunyinStore.setSearchVal(event.target.value)
    };
    query = () => {
        hunyinStore.fetchTableList();
    }
                ...
                <div className='search-content'>
                    ...
                    {hunyinStore.isInput ?
                        <Input onPressEnter={this.query} placeholder="请输入关键字查询" style={{width: 180, marginLeft: 20}}
                               value={hunyinStore.searchVal} onChange={this.handleInputChange}/> :
